<template>
  <div class="toolbar glass border-b border-panel-border flex items-center gap-2 px-4 h-14 shrink-0 z-20">
    ...
    <button
      id="btn-add-interface"
      class="toolbar-btn"
      :class="{ 'opacity-40 cursor-not-allowed': !canAddInterface }"
      :disabled="!canAddInterface"
      title="Add Interface to selected component"
      @click="addInterface"
    >
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <button
      id="btn-add-subcomponent"
      class="toolbar-btn"
      :class="{ 'opacity-40 cursor-not-allowed': !canAddInterface }"
      :disabled="!canAddInterface"
      title="Add Subcomponent to selected component"
    ...
    <button
      id="btn-delete"
      class="toolbar-btn text-danger hover:bg-danger/10"
    ...
    <button id="btn-save" class="toolbar-btn" title="Save diagram as JSON" @click="save">
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
        <path d="M13 13.5H3a.5.5 0 0 1-.5-.5V3a.5.5 0 0 1 .5-.5h8l2.5 2.5V13a.5.5 0 0 1-.5.5Z" stroke="currentColor" stroke-width="1.3"/>
    ...
    <button id="btn-load" class="toolbar-btn" title="Load diagram from JSON" @click="triggerLoad">
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
        ...
        <path d="M8 2v7.5M5.5 7 8 9.5 10.5 7" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
      <span>Load</span>
    </button>
    ...
    <button id="btn-export" class="toolbar-btn" title="Export to PlantUML" @click="exportPlantUML">
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
        <path d="M9.5 2H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V5.5L9.5 2Z" stroke="currentColor" stroke-width="1.3"/>
  ...
  <Teleport to="body">
    <div v-if="plantUMLText" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" @click.self="plantUMLText = null" @keydown.escape="plantUMLText = null">
      <div class="bg-panel border border-panel-border rounded-2xl shadow-2xl w-[680px] max-h-[80vh] flex flex-col" style="box-shadow: 0 24px 64px rgba(0,0,0,0.5)">
        <!-- Header -->
        <div class="flex items-center justify-between px-6 py-4 border-b border-panel-border">
          ...
          <button class="w-8 h-8 flex items-center justify-center rounded-lg text-text-muted hover:text-text-primary hover:bg-surface-hover transition-all cursor-pointer text-lg leading-none" @click="plantUMLText = null">✕</button>
        </div>

        <!-- Code content -->
        <div class="flex-1 overflow-auto p-6">
          <pre class="text-[13px] font-mono text-text-secondary leading-relaxed whitespace-pre-wrap break-words bg-canvas rounded-xl p-5 border border-panel-border">{{ plantUMLText }}</pre>
        </div>
        ...
        <div class="flex justify-center gap-3 px-6 py-4 border-t border-panel-border">
          <button class="toolbar-btn" @click="copyPlantUML">
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
              ...
              <path d="M11 5V3a1 1 0 0 0-1-1H3a1 1 0 0 0-1 1v7a1 1 0 0 0 1 1h2" stroke="currentColor" stroke-width="1.3"/>
            </svg>

      ...
      <div class="bg-panel border border-panel-border rounded-2xl shadow-2xl w-[480px] flex flex-col"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-panel-border">
          <div class="flex flex-col gap-0.5">
            <span class="font-semibold text-text-primary text-base">Apply Interface Rule</span>
            <span class="text-[11px] text-text-muted font-mono">Def. 25</span>
          </div>
          <button
        ...
        <div class="px-6 pt-5 pb-4 flex flex-col gap-4">
          <p class="text-[13px] text-text-secondary">
            Connecting an interface to requirer <strong class="text-text-primary">{{ requirerName }}</strong>.
          </p>

          <div class="flex flex-col gap-1.5">
            <label class="text-[11px] font-semibold uppercase tracking-widest text-text-muted">Provider Component</label>
            <select v-model="selectedProviderId" class="field-input">
              <option value="new">+ Create New Provider Component</option>
              <option v-for="comp in availableProviders" :key="comp.id" :value="comp.id">
                {{ comp.name }}
              </option>
            </select>
          </div>
          ...
          <div class="flex flex-col gap-2">
            ...
            <label class="flex items-start gap-3 p-4 rounded-lg border border-panel-border hover:bg-surface-hover cursor-pointer transition-all"
              ...
              <div class="flex flex-col gap-0.5">
                ...
                <span class="text-[11px] text-text-muted">Connect via top-level OR gate. Requirer fails whenever provider fails.</span>
              </div>
            </label>
...
const selectedProviderId = ref<string>('new')
...
const availableProviders = computed(() => {
  const existingProvidedIds = new Set(
    diagramStore.interfaces
      .filter(i => i.requiredComponentId === props.requirerId)
      .map(i => i.providedComponentId)
  )
  return diagramStore.components.filter(
    c => c.id !== props.requirerId && !existingProvidedIds.has(c.id)
  )
})

const freeTargets = computed(() =>
  cftStore.getFreeTargetsForComponent(props.requirerId)
)
...
function runPreCheck() {
  ...
  const providerId = selectedProviderId.value === 'new' ? null : selectedProviderId.value
  checkResult.value = diagramStore.checkCanApplyInterfaceRule(props.requirerId, providerId, attachmentPointId)
}

onMounted(runPreCheck)
watch([selectedProviderId, mode, selectedTarget], runPreCheck)

function apply() {
  applying.value = true
  const attachmentPointId = mode.value === 'target' ? selectedTarget.value || undefined : undefined
  let providerId = selectedProviderId.value
  if (providerId === 'new') {
    const parent = diagramStore.components.find(c => c.id === props.requirerId)
    const newProv = diagramStore.addComponent(
      null,
      parent ? parent.x + parent.width + 160 : 400,
      parent ? parent.y : 200,
    )
    diagramStore.updateComponent(newProv.id, { name: 'Provider' })
    providerId = newProv.id
  }
  const result = diagramStore.applyInterfaceRule(props.requirerId, providerId, attachmentPointId)
